...
def find_qemu():
    ...
    real_path = os.path.realpath(os.path.join(os.path.dirname(__file__), default))
...
def get_entry_and_init_sp(config, base_dir):
    ...
    init_filename = get_memory_filename(config['memories'][init_memory], base_dir)
...
def emulate_binary(config, base_dir, target_name=None, log_basic_blocks=None,
    ...
    record_memories = []
    for name, memory in config['memories'].items():
    ...
    for name, per in config['peripherals'].items():
   ...
   # Setup Intecepts
    ...
    qemu.set_vector_table_base(0x08000000) #TODO Change to be read from config
...
def override_addresses(config, address_file):
    ...
    with open(address_file, 'rb') as infile:
        ...
        func2addr_lut = {}
        for key, val in addr2func_lut.items():
    ...
    for intercept in config['intercepts']:
        ...
        if f_name in func2addr_lut:
            intercept['addr'] = (func2addr_lut[f_name] & 0xFFFFFFFE) #clear thumb bit
            log.info("Replacing address for %s with %s " % (f_name, hex(func2addr_lut[f_name])))
