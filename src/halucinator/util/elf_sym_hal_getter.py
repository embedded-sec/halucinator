...
def build_addr_to_sym_lookup(binary):
    ...
    for addr, sym in loader.main_object.symbols_by_addr.items():
        if sym.is_function:
            start_addr = addr & 0xFFFFFFFE # Clear Thumb bit
            for a in xrange(start_addr, start_addr+sym.size, 2):
                sym_lut[a] = sym
    return sym_lut
...
def get_functions_and_addresses(binary):
    ...
    for symbol in loader.symbols:
         if symbol.is_function:
            functions[symbol.name] = symbol.rebased_addr & 0xFFFFFFFE # Clear Thumb bit
    return functions
