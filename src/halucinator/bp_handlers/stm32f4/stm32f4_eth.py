...
class STM32F4Ethernet(BPHandler):
    ...
    def handle_tx(self, qemu, bp_addr):
        ...
        tx_frame_ptr = qemu.read_memory(tx_dma_desc + 8, 4, 1)  # heth->TXDesc->Buffer1Addr
        frame = qemu.read_memory(tx_frame_ptr, 1, length, raw=True)  # *(heth->TXDesc->Buffer1Addr)
    ...
    def handle_rx(self, qemu, bp_addr):
        ...
        if frame is not None:
            if avatar.recorder is not None:
                avatar.recorder.save_state_to_db('HAL_ETH_GetReceivedFrame', is_entry=True)
            ...
            FrameInfo = struct.pack('<IIIII', RxDesc_ptr, RxDesc_ptr, 1, len(frame), BuffAddr)
            ...
            if avatar.recorder is not None:
                avatar.recorder.save_state_to_db('HAL_ETH_GetReceivedFrame', is_entry=False)
