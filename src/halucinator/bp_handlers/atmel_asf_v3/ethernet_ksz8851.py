...
class Ksz8851Eth(BPHandler):
    ...
    def fifo_write(self, qemu, bp_addr):
        ...
        self.frame.append(qemu.read_memory(qemu.regs.r0,1,qemu.regs.r1, raw=True))
        #
        return True, None
    ...
    def fifo_read(self, qemu, bp_addr):
        ...
        log.info("Frame Received: Delay %s, Frame: %s" % (str(time.time()-rx_time), binascii.hexlify(frame[:10])))
        ...
        # front of buffer
        qemu.write_memory(buf_ptr + Ksz8851Eth.PADDING, 1, frame, length, raw=True)
        return True, None
