...
class RF233Radio(BPHandler):
    ...
    def send(self, qemu, bp_addr):
        ...
        log.debug("Send")
        frame = qemu.read_memory(qemu.regs.r0, 1, qemu.regs.r1  & 0xFF, raw=True)    
    ...
    def sram_read(self, qemu, bp_addr):
        ...
        if self.model.has_frame() is not None:
            ...
            if len(frame) <= buf_size:
                log.debug("Writing Buffer to memory %s " % (binascii.hexlify(frame)))
    ...
    def trx_reg_read(self, qemu, bp_addr):
        ...
        if reg == RF233_REG_IRQ_STATUS:
            ...
            log.debug("trx_reg_read: %s Unimplemented register returning  0, %s" % (reg, hex(qemu.regs.lr)))
            ret_val = 0
        return True, ret_val 
    ...
    def trx_spi_init (self, qemu, bp_addr):
        ...
        qemu.regs.r0 = qemu.avatar.callables['AT86RFX_ISR'] | 1 #Set Thumb bit
        qemu.regs.r1 = 0 
        qemu.regs.r2 = 0
        qemu.regs.pc = qemu.avatar.callables['extint_register_callback'] | 1 #Set Thumb bit
        return False, None
