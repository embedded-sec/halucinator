...
class Recording(object):
    ...
    def _get_value(self, addr, size, state):
        for offset, mem in state['memory'].items():
    ...
    def get_param(self, param_num, size=4):
        ...
        if param_num < 4 and param_num >=0:
            ...
            val = struct.unpack("<I",self.get_before_value(sp+sp_offset, 4))[0]
            return val
...
class App(object):
    ...
    def display_recording(self, record):
        ...
        self.add_children(record, sym_reader, funct_die, parent_values, ele, deref)
        ...
        for i, p_die in enumerate(sym_reader.get_parameter_dies(funct_die)):
            ...
            deref = {'before':set(), 'after':set()}
            self.add_children(record, sym_reader, p_die, parent_values, row, deref)
